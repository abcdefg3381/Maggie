 ...
 * JCalendarPopup). Note: This button doesn't use some of the global constants
 * and methods because it is used in other programs where they are not
 * available.
 * 
 * @author Administrator
 * @version 1.0.0
 */
public class JCalendarButton extends JButton implements PropertyChangeListener, ActionListener {
	/**
	...
	public JCalendarButton() {
		...
		try {
			Icon icon =
					new ImageIcon(cl.getResource("images/buttons/"
							+ JCalendarPopup.CALENDAR_ICON + ".gif"));
			this.setIcon(icon);
		} catch (Exception ex) {
			this.setText("change");
		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this) {
			...
			JCalendarPopup popup =
					JCalendarPopup.createCalendarPopup(this.getDateParam(), dateTarget, this,
							m_strLanguage);
	...
	public void init(String strDateParam, Date dateTarget, String strLanguage) {
		...
		this.setText(df.format(m_dateTarget));
	}
