 ...
 * @version 1.0.0
 */
public class JCalendarPopup extends javax.swing.JPanel implements MouseListener {
	/**
	 ...
	 *            The calling button.
	 */
	public static JCalendarPopup createCalendarPopup(String strDateParam, Date dateTarget,
			Component button) {
		return JCalendarPopup.createCalendarPopup(null, dateTarget, button, null);
	}
	...
	public void init(String strDateParam, Date dateTarget, String strLanguage) {
		...
		if (dateTarget == m_dateNow) {
			m_dateSelected = m_dateTarget; // If they want a default date, make
			// the time 12:00
		}
	}
	...
	public void layoutCalendar(Date dateTarget) {
		...
		m_iTargetComponent =
				(int) ((m_dateTarget.getTime() - dateCalendarFirstDate.getTime()) / KMS_IN_A_DAY);
	...
	public void mouseClicked(MouseEvent evt) {
		...
		if (popupMenu != null) { // I'm not sure this is the correct code, but
			// here it is!
			...
			if (invoker != null)
				if (m_bTransferFocus) {
					invoker.transferFocus(); // Focus on next component after
					// invoker
				}
		}
